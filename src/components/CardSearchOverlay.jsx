...
export default function CardSearchOverlay({ mode = 'raw', onClose }) {
    ...
    return (
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {items.map((card) => (
            <CardTile key={card.id} card={card} price={prices[card.id]} onAdd={setPicked} />
          ))}
        </div>
      </section>
    )
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-[#0b1020]">
      {/* Header + search box */}
      <div className="sticky top-0 z-10 border-b border-white/10 bg-[#0b1020]/95 p-4 pt-[calc(1rem+env(safe-area-inset-top))] backdrop-blur">
        ...
        <input
          ...
          autoFocus
          className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-gray-100 placeholder-gray-500 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
        />
      </div>
